...
```{css, echo=FALSE}
.main-container { max-width: 100% !important; padding: 0 10%; }
...
trees <-  trees_layerNames %>%
    lapply(function(x) {
        st_read(paste0("../LTFEM_Veg/data/classified/geojson/", x, ".geojson")) %>%
            st_transform(4326) %>%
            mutate(radius = DBH / 200) #adjust denominator for scale
    }) %>%
    set_names(trees_layerNames)

seedlings_layerNames <- c("WYK_quadrat_boundaries_reprojected", #saved in classified folder
  "NSSF_quadrat_boundaries_reprojected")                        #contains seedlings data
seedlings <- seedlings_layerNames %>% 
    lapply(function(x) {
        st_read(paste0("../LTFEM_Veg/data/classified/geojson/", x, ".geojson")) %>% 
                    st_transform(4326)
    }) %>%
    set_names(seedlings_layerNames)
...
rm(trees_layerNames, seedlings_layerNames, boundaries_layerNames)
```
...
leaflet(height = "80vh", options = leafletOptions(maxZoom = 30, minZoom = 12)) %>%
    addProviderTiles(providers$Esri.WorldTopoMap, options = providerTileOptions(maxZoom = 30)) %>%
    ...
    addPolygons(
        data = seedlings$WYK_quadrat_boundaries_reprojected, #take note that data is extracted from seedlings instead of boundaries
        color = "red",
        weight = 2,
        fill = TRUE,
        group = "WYK Seedling Plots",
        popup =  ~paste0("<b>Quadrat ID:</b>", quadratID,
                         "<br><b>Species:</b>", seedlings)
    ) %>% 
    
        addPolygons(
        data = seedlings$NSSF_quadrat_boundaries_reprojected, #take note that data is extracted from seedlings instead of boundaries
        color = "red",
        weight = 2,
        fill = TRUE,
        group = "NSSF Seedling Plots",
        popup =  ~paste0("<b>Quadrat ID:</b>", quadratID,
                         "<br><b>Species:</b>", seedlings)
        ) %>% 
    ...
    addLayersControl(
        overlayGroups = c("Mandai Saplings", "Mandai Trees", "Mandai Big Trees",
                          ...
                          "WYK Seedling Plots", "WYK 10x10 Plots", "WYK 20x20 Plots", "WYK 40x40 Plots",
                          "NSSF Saplings", "NSSF Trees", "NSSF Big Trees",
                          "NSSF Seedling Plots", "NSSF 10x10 Plots", "NSSF 20x20 Plots", "NSSF 40x40 Plots",
    ...
    addLegend(
        ...
        labels = c( "Seedlings", "Saplings", "Trees", "Big Trees"),
        position = "topright",
        title = "Legend"
    )
```
